...
input_im = imread('data/input_12.JPG');
...
% cropTop = floor(imHeight * .1);
% cropBottom = imHeight - cropTop;
% cropLeft = floor(imWidth * .1);
...
[rows,cols] = find(g_bw); %// Step #1

top_left_row = min(rows)*3  %// Step #2
top_left_col = min(cols)*3
bottom_right_row = max(rows)*3
bottom_right_col = max(cols)*3
